...
class _ChatState extends State<Chat> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadDirectChats() async {
    if (widget.receiverId == null) return; // Ensure receiverId isn't null
    setState(() {
      _isLoading = true;
    });
    try {
  ...
  Future<void> _loadEventChats() async {
    if (widget.eventId == null) return; // Ensure eventId isn't null
    setState(() {
      _isLoading = true;
    });
    try {
            ...
            itemBuilder: (context, index) {
              ...
              return _buildMessageTile(
                ...
                message['sender']['image'] ??
                    'assets/sidra.png', // Adjust to show sender's image
              );
            },
          ),
        ),
  ...
  Widget _buildMessageTile(String title, String subtitle, String imagePath) {
    return ListTile(
      leading: CircleAvatar(
        backgroundImage: NetworkImage(imagePath), // Use NetworkImage for images
      ),
      title: Text(title),
